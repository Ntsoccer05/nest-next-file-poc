...
Issue が開かれたときに自動実行されるラベル分類ロジック。`/label-issue` コマンドで実行。

### ラベル付与ルール

| ラベル | 判断基準 |
|---|---|
| **bug** | バグ報告、エラーログ、期待動作との不一致 |
| **feature** | 新しい機能のリクエスト |
| **enhancement** | 既存機能の改善・最適化リクエスト |
| **documentation** | README・CLAUDE.md・コメント等の情報不足 |
| **question** | 使い方の質問、サポート依頼 |

### 分類プロセス

1. `.claude/commands/label-issue.md` に定義されたコマンドで実行
2. 利用可能なラベル一覧を取得（`./scripts/gh.sh label list`）
3. Issue の タイトル・本文から適切なラベルを判定
4. 上記ルールに基づいて対応するラベルを付与（複数可）
5. コメント投稿は行わない（ラベル付けのみ）

### 注意点

- Issue が明確でない場合は `question` で保留
- 既存 Issue との重複がないか確認可能な場合は確認
- コメント投稿は厳禁（ラベル付与のみに専念）
